...
// Default design for quotes: docTitle is DEVIS instead of FACTURE
const QUOTE_INITIAL_DESIGN = { ...INITIAL_DESIGN, docTitle: 'DEVIS' };
...
export default function QuoteEditor() {
  ...
  const [design, setDesign] = useState(QUOTE_INITIAL_DESIGN);
  ...
  }, [company, client, details, sections, extras, notes, design, loadingQuote, draftKey]);
  ...
  useEffect(() => {
    ...
    api.get(`/quotes/${id}`)
      .then(r => {
        const q = r.data;
        setClient({ id: q.clientId, name: q.client?.name || '', email: q.client?.email || '', phone: q.client?.phone || '', address: q.client?.address || '', city: q.client?.city || '' });
        if (q.companyData) setCompany(c => ({ ...c, ...q.companyData }));
        setDetails(d => ({
          ...d,
          number: q.number || '',
          date: q.issueDate ? q.issueDate.split('T')[0] : d.date,
          dueDate: q.validUntil ? q.validUntil.split('T')[0] : '',
          validity: q.validUntil ? '' : d.validity,
        ...
        setExtras(e => ({
          ...e,
          taxRate: q.taxRate ?? 19.25,
          discount: q.discount ?? 0,
          labour: q.labour ?? 0,
          extra: q.extra ?? 0,
        }));
        setNotes({ conditions: q.notes || '', footer: q.footer || '' });
        setDesign(d => ({ 
          ...d, 
          docTitle: 'DEVIS',
          template: q.templateType || 'elegant', 
          palette: q.palette || 'skyblue',
          font: q.font || 'Inter',
          watermark: q.watermark || d.watermark,
          stamp: q.stamp || d.stamp
        }));
      ...
      .catch(() => toast.error(lang === 'fr' ? 'Devis introuvable' : 'Quote not found'))
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        currency: details.currency, validUntil: details.dueDate || null,
  ...
  const handleExportPDF = () => {
    exportInvoicePDF(document.getElementById('quote-preview-container'), {
      number: details.number || `DEV-${new Date().getFullYear()}-XXXX`,
    });
  };
  ...
  const previewNode = (
    <div style={{ flex: 1, overflowY: 'auto', background: '#cbd5e1', padding: isMobile ? '12px 8px' : '20px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
      ...
      <div id="quote-preview-container" style={{ transform: isMobile ? `scale(${Math.min(1, (window.innerWidth - 16) / 595)})` : 'none', transformOrigin: 'top center', marginBottom: isMobile ? `-${842 * (1 - Math.min(1, (window.innerWidth - 16) / 595))}px` : 0 }}>
  ...
  const formNode = (
    <div style={{ width: isMobile ? '100%' : 380, display: 'flex', flexDirection: 'column', borderRight: isMobile ? 'none' : '1px solid var(--border)', background: 'var(--bg2)', flexShrink: 0 }}>
      ...
      <div style={{ padding: '12px 14px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
        <button className="btn-icon" onClick={() => navigate('/quotes')}><ArrowLeft size={16} /></button>
        <span style={{ flex: 1, fontWeight: 800, fontSize: isMobile ? 14 : 15, fontFamily: 'Inter,sans-serif', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {isEditing ? (lang === 'fr' ? `Modifier ${details.number || ''}` : `Edit ${details.number || ''}`) : (lang === 'fr' ? 'Nouveau devis' : 'New Quote')}
        </span>
